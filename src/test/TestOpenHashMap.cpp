...
void testBasic1()
{
    ...
    std::cout << "OpenHashMap Internal\n";
    map.printInternal(std::cout);

    std::cout << "OpenHashMap: " << map << "\n";
    std::cout << "OpenHashMap: [";
    for (OpenHashMap<int,int>::iterator it = map.begin(); it != map.end(); ++it) {
	if (it != map.begin()) {
	    std::cout << ", ";
	}
